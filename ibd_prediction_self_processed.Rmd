...
meta <- meta[LOCATION!="Ctrl"]
meta[,CEL_FILE:=gsub('.CEL', '',CEL_FILE)]
...
expr <- fread("/nfs/scratch/gene_signature/eset_symbol.txt")
expr_key <- "V1"
colnames(expr)[1] <- expr_key
setkeyv(expr, expr_key)                             # set key

colnames(expr) <- gsub(".*P_", "P_", colnames(expr))
...
expr <- as.matrix(expr, rownames="V1")
...
unique_ortholog_mapping <- unique_ortholog_mapping[!duplicated(ortholog_name)]
...
column_ha = HeatmapAnnotation(GENDER = meta_M0M6$GENDER, LOCATION = meta_M0M6$LOCATION, RECURRECE=meta_M0M6$RECURRENCE)
